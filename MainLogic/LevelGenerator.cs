...
public class LevelGenerator : MonoBehaviour
{
    ...
    void OnEnable()
    {
        // Создаем плитки и распределяем их по слоям
        CreateTiles();
        ShuffleTiles();
        LayoutTiles();
        AssignLayers();
...
private void AssignLayers()
{
    string[] layerNames = { deacLevel0Name, deacLevel1Name, deacLevel2Name, deacLevel3Name, deacLevel4Name, deacLevel5Name };
    int totalLayers = layerNames.Length;
    int tilesPerLayer = totalTileCount / totalLayers;
    ...
    for (int i = 0; i < tiles.Count; i++)
    {
        ...
        if (layer != -1)
        {
            ...
            // Настраиваем прозрачность плитки
            SetTransparency(tiles[i], 0.5f); // Прозрачность на 50%

            // Настраиваем кликабельность
            MakeTilesNonClickable();
        }
        else
        {
            Debug.LogError("Layer not found: " + layerName);
...
private void MakeTilesNonClickable()
{
    string[] layerNames = { deacLevel0Name, deacLevel1Name, deacLevel2Name, deacLevel3Name, deacLevel4Name, deacLevel5Name };

    foreach (var tile in tiles)
    {
        // Определяем, какой слой должен быть у плитки
        string layerName = layerNames[Random.Range(0, layerNames.Length)];
        int layer = LayerMask.NameToLayer(layerName);

        // Устанавливаем слой
        tile.layer = layer;

        // Убедитесь, что данный слой не входит в Raycast и UI взаимодействия
        // В вашем случае добавьте исключение слоя в обработке Raycast или используйте EventSystem.
    }
}
private void SetTransparency(GameObject tile, float alpha)
{
    ...
    if (renderer != null)
    {
        foreach (Material mat in renderer.materials)
        {
            ...
            color.a = alpha;
            mat.color = color;
            mat.SetFloat("_Mode", 2);  // Устанавливаем на прозрачный режим
